    ...
    fun showEditProfileDialog(
        ...
        return MaterialAlertDialogBuilder(context)
            ...
            .setPositiveButton(context.getString(R.string.save)) { dialog, _ ->
                validateAndSaveProfileChanges(context, dialogView, userRepository, callback)
                dialog.dismiss()
            }
        ...
        when {
            ...
            location.isEmpty() -> callback.onError("Выберите локацию")
                ...
                if (emailChanged(email, auth.currentUser?.email ?: "")) {
                    showPasswordDialog(context, name, email, address, phone, location, userRepository, callback)
                } else {
                    updateProfileData(name, email, address, phone, location, userRepository, callback)
                }
            }
        }
    }
    ...
    private fun showPasswordDialog(
        ...
        dialog.setOnShowListener {
            ...
            positiveButton.setOnClickListener {
                ...
                when {
                    password.isEmpty() -> {
                        passwordBinding.textInputLayout.error = context.getString(R.string.enter_password)
                    }
                    password.length < 6 -> {
                        passwordBinding.textInputLayout.error = context.getString(R.string.password_too_short)
                    }
                    else -> {
        ...
        kotlinx.coroutines.GlobalScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            ...
            kotlinx.coroutines.GlobalScope.launch(kotlinx.coroutines.Dispatchers.Main) {
                if (successCount == totalUpdates) {
                    ...
                    callback.onError("Часть данных сохранена")
                }
            }
        }
    }
